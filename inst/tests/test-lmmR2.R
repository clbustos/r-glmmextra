...
x2.g.i<-x2.g[gg2]
b2<-x2.g.i+e2.g[gg2] # varianza teorica de b2->2
...
x.g.i<-x.g[gg]
b<- x.g.i+e.g[gg] # varianza teorica de b->2


x.i<-rnorm(m*reps.n1*reps.n2)
e.i<-rnorm(m*reps.n1*reps.n2) # varianza del residuo -> 1
y<-b+b2+x.i+e.i

m.null<-lmer(y~(1|gg2/gg)) # varianza de b=2, varianza del residuo=2
m.full<-lmer(y~(1+x.i|gg2/gg)+x.g.i+x.i) # varianza de b=1, residuo=1 -> predigo a nivel global y local
